...
and c.idCarrera = ".$idCarrera." order by 2";
	...
	function get_horarios_legajo($Legajo){
	   $con = new DBManager;
	   if($con->conectar()==true){
		...
		$query = "select 
			...
			from Alumno_Horarios ah,
			     Horario_Semana hs,
			     SegmentoHorario sh,
			     DiaSemana ds 
			where ah.idAlumno = (select idAlumno from Alumno where Legajo =  ".$Legajo.")
			and hs.idHorario_Semana = ah.idHorario_Semana
			  and hs.idSegmentoHorario = sh.idSegmentoHorario
			  and hs.idDiaSemana = ds.idDiaSemana
			order by ds.DiaSemana,sh.HoraDesde";
					...
					where m.idMateria = c.idMateria 
					  ...
					  and s.idSede = us.idSede";
						...
						where m.idMateria = c.idMateria 
						  ...
						  and exists (select * from Alumno_Horarios ah where ah.idAlumno = (select idAlumno from Alumno where Legajo =  ".$Legajo.")
						                and ah.idHorario_Semana = hs.idHorario_Semana)
						  and exists (
						  SELECT m1.idMateria,
						       m1.Nombre as Nombre_Materia,
						       m1.CargaHoraria,
